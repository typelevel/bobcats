...
/**
 * the JS Crypto API does not give us the tools to parse a PEM in a way that we end up with a
 * pure key. So we will just assume the PEM given is correct and build the specs from that.
 *
 * note: this class uses no JS libs, so it could be the default
 *
 * the whole problem here is that we are forced to pass the encoding of the key, where that
 * information is located IN the PKCS8 key. JS WebCrypto API cannot parse PKCS1 keys where the
 * that information is located in the ----- headers ----- (which we could parse) because the JS
 * Crypto API does not understand that format.
 *
 * So as a result this library is not very helpful for use on the Web, where one may come across
 * keys in PKCS1 format or keys in PKCS8 format and not know the key type
 *
